...
export const createAccount = createAsyncThunk("/student", async (data) => {
  console.log(data);
  try {
    const response = axiosInstance.post("/student", data);
...
export const LoginAccount = createAsyncThunk("/student/login", async (data) => {
  try {
    console.log(data);
    const response = axiosInstance.post("/student/login", data);
    //    console.log(data?.da);
    toast.promise(response, {
...
export const me = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      ...
      console.error(error);
      toast.error(
        error?.response?.data?.message || "Failed to view Your Profile"
      );
  ...
  extraReducers: (builder) => {
    builder
      .addCase(LoginAccount.fulfilled, (state, action) => {
        ...
        (state.isLoggedIn = true), console.log(action?.payload?.user);
